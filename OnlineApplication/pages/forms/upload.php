...
$conn = mysqli_connect("localhost", "ekhonnec_vakhandli_group", "vakhandli_group", "ekhonnec_vakhandli_group") or die("Failed to connect to database");
...
if (isset($_POST['Save'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO documents (Identity_Type, PolicyOrId, file_path, agreement, title, Comment, _On) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssss", $typeCategory, $regNum, $file_path, $type, $Title, $comment, $Date13);
    $stmt->execute();
    ...
    // Retrieve the uploaded documents from the database table
}
...
<form class="forms-sample" method="post" action="asset.php">
  ...
  <div class="col-sm-9">
    <?php
      ...
      // Select all documents from the database
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr><td>" . $row['title'] . "</td><td><a href=\"" . $row['file_path'] . "\">Download</a></td><td><a href='delete-file.php?del_id=$row[id]''  style='display: inline-block; padding: 8px 12px; background-color: #dc3545; color: #fff; border-radius: 4px; text-decoration: none;'>Delete</a></td></tr>";
      }
      echo "</table>";
