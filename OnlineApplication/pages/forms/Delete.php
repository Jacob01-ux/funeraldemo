...
$id = $_GET['id'];
$stmt = $pdo->prepare('SELECT * FROM ' .$table. ' WHERE '.$firstColValue.' = ?');
$stmt->execute([$id]);
$product = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $stmt = $pdo->prepare('DELETE FROM ' .$table. ' WHERE '.$firstColValue.' = ?');
  $stmt->execute([$id]);
...
<head>
  ...
  <title>Vakhandli Group | Delete</title>
  <!-- plugins:css -->
...
<form method="post">
  ...
  <?php $first_four_columns = array_slice($columns, 0, 4); ?>
  <?php foreach ($first_four_columns as $key): ?>
    <div class="form-group">
      <label for="<?= htmlspecialchars($key) ?>"><?= htmlspecialchars($key) ?></label>
      <input type="text" class="form-control" id="<?= htmlspecialchars($key) ?>" name="<?= htmlspecialchars($key) ?>" value="<?= htmlspecialchars($product[$key]) ?>">
    </div>
  <?php endforeach; ?>
  <button type="submit" class="btn btn-primary" id="submit-btn">Delete</button>
