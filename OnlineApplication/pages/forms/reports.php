...
<head>
  ...
  <title>Vakhandli Group | new client</title>
  <!-- plugins:css -->
...
<div class="main-panel">        
      ...
      <div class="form-group">
                      <label for="m_status"></label>
                        <select class="form-control" id="repots" name="reports"  required>
                          
                          <option value="Sales">choose an option</option>
  ...
  <script>
    ...
    /*
    $('#repots').on('change', function() {
			$.ajax({
			url:"sales_report.php",
			method:"POST",
			data:{cat:$('#reports').val()},
...
*/
  ...
  $('#repots').on('change', function() {
    ...
    var inputValue = $('#repots').val();
    $.ajax({
      url: 'sales_report.php',
      method: 'POST',
      data: {cat: inputValue},
      success: function(response) {
        $('#result1').html(response); // do something with the response
      },
      error: function(xhr, status, error) {
        console.log(xhr.responseText); // show the error message
      }
    });
  });
});
