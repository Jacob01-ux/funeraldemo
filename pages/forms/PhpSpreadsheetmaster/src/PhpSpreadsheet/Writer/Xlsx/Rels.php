...
namespace PhpSpreadsheetmaster\src\PhpSpreadsheet\Writer\Xlsx;

use PhpSpreadsheetmaster\src\PhpSpreadsheet\Reader\Xlsx\Namespaces;
use PhpSpreadsheetmaster\src\PhpSpreadsheet\Shared\XMLWriter;
...
class Rels extends WriterPart
{
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\PhpSpreadsheetmaster\src\PhpSpreadsheet\Worksheet\Worksheet $worksheet, $worksheetId = 1, $includeCharts = false, $tableRef = 1)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        if (($worksheet->getDrawingCollection()->count() > 0) || (count($charts) > 0) || $drawingOriginalIds) {
            ...
            // (! synchronize with \PhpSpreadsheetmaster\src\PhpSpreadsheet\Writer\Xlsx\Worksheet::writeDrawings)
    ...
    private function writeUnparsedRelationship(\PhpSpreadsheetmaster\src\PhpSpreadsheet\Worksheet\Worksheet $worksheet, XMLWriter $objWriter, string $relationship, string $type): void
    {
        $unparsedLoadedData = $worksheet->getParentOrThrow()->getUnparsedLoadedData();
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\PhpSpreadsheetmaster\src\PhpSpreadsheet\Worksheet\Worksheet $worksheet, &$chartRef, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            ...
            if (
                $drawing instanceof \PhpSpreadsheetmaster\src\PhpSpreadsheet\Worksheet\Drawing
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships(\PhpSpreadsheetmaster\src\PhpSpreadsheet\Worksheet\Worksheet $worksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    public function writeVMLDrawingRelationships(\PhpSpreadsheetmaster\src\PhpSpreadsheet\Worksheet\Worksheet $worksheet): string
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
