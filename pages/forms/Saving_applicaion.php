...
$inc_date = new DateTime($inc_date);
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO incoming_clients (Customer, _ID, Number, Gender, Package, PremiumCoverAmount, joining,payment_method, Policy, status, Covered, Group_Name, Category, email, Address, Product_Added_Benefit, extended_members, Marital_Status, Inception_date, Payment_Date, Period, Activation_date, _By, Nationality, languageOf_com) 
                        VALUES (:Customer, :ID_no, :Number, :Gender, :Package, :PremiumCoverAmount,:Joining,:payment_method, :Policy, :status, :Covered, :cat, :Category, :email, :Address, :Product_Added_Benefit, :extended_members, :Marital_Status, :Inception_date, :Payment_Date, :Period, :Activation_date, :_By, :Nationality, :languages_)");
    ...
    $stmt->bindParam(':PremiumCoverAmount', $PremiumCoverAmount);
  	$stmt->bindParam(':Joining', $joining);
...
if ($m_status == "Married" || $m_status == "Partner") {
    try {
        ...
        $save_extended = $conn1->prepare("INSERT INTO incoming_spouse (Policy, spouse_Name, spouse_Surname, spouse_Number, spouse_ID, _By) 
...
$massage = "Hello, $Customer\n\nCongratulations and welcome to $cat. Thank you for applying. Your application has been received, and we appreciate your patience.Please wait for approximately 24 hours to receive a response from us. Once approved, your policy will be registered under $cat. Your policy number is  $policy_no and your policy will be activated on $Activation_date\n\n*Use the above policy number for all your queries.\n\n Thank you and have a wonderful day.";
...
$business_o_massage = "Hello,\n\nYou have new incoming client. Click to approve/reject application https://mandhagri.co.za/21/Sales_Legend: \n\nSales Lagend | auto response";
...
$sendData = "{ 'messages' : [ { 'content' : '$business_o_massage', 'destination' : '$business_o' } ] }";
$options = array(
    'http' => array(
        'header'  => array("Content-Type: application/json", $authHeader),
        'method'  => 'POST',
