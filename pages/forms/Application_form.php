...
function updateOutput() {
  ...
  $.ajax({
   ...
   <div class="form-group">
    <label for="product">Joining Fee<a style="color: red; font-size: large;">*</a></label>
    <select class="form-control" id="Joining" name="Joining">
  <?php
    $query20 = mysqli_query($conn, 'SELECT Joining_fee FROM policies');
    while ($row = mysqli_fetch_assoc($query20)) {
      echo "<option>" . $row['Joining_fee'] . "</option>";
    }
  ?>
</select>
...
document.getElementById("policy_no1").addEventListener('input', function() {
    ...
    function sendValue() {
        ...
        xhr.open("POST", "Saving_applicaion.php", true);
