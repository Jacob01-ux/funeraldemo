...
$u3 = $result3->fetch_assoc();

    //Saving and Securing Data
    if (isset($_POST['Save'])) {
        // Take HTML form data and assign to variables
        ...
        if(isset($_FILES['fileUpload'])){
            ...
            $extensions = array("pdf","doc","docx","jpg","jpeg","png","txt");

            if (in_array($file_ext, $extensions) === false){
                echo "Extension not allowed, please choose a PDF, DOC, DOCX, JPG, JPEG, or PNG file.";
                exit; // Stop further processing if file extension is not allowed
            } else {
        ...
        $stmt = $conn->prepare("INSERT INTO documents (Identity_Type, PolicyOrId, file_path, agreement, title, Comment, _By, _On) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssss", $typeCategory, $regNum, $file_path, $type, $Title, $comment, $u3["names"], $Date13);
        $stmt->execute();
...
<head>
  ...
  <title>Funeral Demo | documents</title>
  <!-- plugins:css -->
...
<form class="forms-sample" method="post" action="asset.php">
  <div class="form-group row">
    ...
    <?php
      // Connect to the database
      $conn = mysqli_connect("localhost", "ekhonnec_JeudfraBS", "JeudfraBS33@", "ekhonnec_JeudfraBS") or die("Failed to connect to the database");

      // Sanitize and validate the $u variable before using it in the query
      $userId = mysqli_real_escape_string($conn, htmlspecialchars($u["names"]));

      // Select all documents from the database for the specific user
      $sql = "SELECT * FROM documents WHERE _By = '$userId'";
      $result = mysqli_query($conn, $sql);

      // Check for errors during the query execution
      if (!$result) {
        die("Failed to execute the query: " . mysqli_error($conn));
      }

      // Display the uploaded documents in a table
      echo "<table>";
      echo "<tr><th>File Name</th><th>Download</th><th>Action</th></tr>";

      while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr>";
        echo "<td>" . htmlspecialchars($row['title']) . "</td>";
        echo "<td><a href=\"" . htmlspecialchars($row['file_path']) . "\">Download</a></td>";
        echo "<td><a href='delete-file.php?del_id=" . htmlspecialchars($row['id']) . "' style='display: inline-block; padding: 8px 12px; background-color: #dc3545; color: #fff; border-radius: 4px; text-decoration: none;'>Delete</a></td>";
        echo "</tr>";
      }
...
<div style="text-align: center;">
          <!--button type="submit" id="sub" class="btn btn-primary mr-2" name="save_Products">Save</button-->
    </form>
  
    <button type="button" class="btn btn-light" onclick="location.href='Document_Portal.php'">Back</button>
</div>
<div id='result'></div>
<br><br>
</div> 
