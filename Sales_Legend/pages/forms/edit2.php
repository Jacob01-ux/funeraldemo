...
// start the session and get table and column values
session_start();
$table = $_SESSION['table'];
$firstColValue = $_SESSION['column'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // retrieve the ID and the values of the other columns from the form
    $id = $_POST['id'];
    $values = array();
    foreach ($columns as $column) {
        if ($column != $firstColValue) {
    ...
    $sql .= " WHERE $firstColValue=:id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindValue(':id', $id);
    $i = 0;
    foreach ($values as $key => $value) {
        $stmt->bindValue(':value_' . $i, $value);
        $i++;
    }
    $result = $stmt->execute();
    ...
    if (!$result) {
        $error_message = "Error updating row: " . $stmt->errorInfo()[2];
    } else {
        $success_message = "Row updated successfully";
    }
}

// retrieve the ID of the row to be edited
$id = $_GET['id'];
$sql = "SELECT * FROM $table WHERE $firstColValue=:id";
$stmt = $pdo->prepare($sql);
$stmt->bindValue(':id', $id);
$stmt->execute();
$row = $stmt->fetch();
         ...
         <!--Side Menu Start-->
          ...
          <li class="nav-item">
            ...
            <div class="collapse" id="form-elements">
              <ul class="nav flex-column sub-menu">
                ...
                <li class="nav-item"><a class="nav-link" href="AssetTable.php">Reports</a></li>
              </ul>
            </div>
          </li>
          <li class="nav-item">
          ...
          <li class="nav-item">
            ...
            <div class="collapse" id="tables">
              <ul class="nav flex-column sub-menu">
                <li class="nav-item"> <a class="nav-link" href="Asset_Register.html">Asset Register and Disposal</a></li>
          ...
          <li class="nav-item">
            ...
            <div class="collapse" id="maps">
              <ul class="nav flex-column sub-menu">
                <li class="nav-item"> <a class="nav-link" href="prod.php">Add Policies</a></li>
                <li class="nav-item"> <a class="nav-link" href="AssetTable.php">Available Policies</a></li>
                <li class="nav-item"> <a class="nav-link" href="AssetTable.php">Policy Reports</a></li>
              </ul>
            </div>
...
<!-- form to edit the row -->
<form method="post">
  <input type="hidden" name="id" value="<?= htmlspecialchars($id) ?>">
  <?php foreach ($columns as $key): ?>
    <div class="form-group">
      <label for="<?= htmlspecialchars($key) ?>"><?= htmlspecialchars($key) ?></label>
      <input type="text" class="form-control" id="<?= htmlspecialchars($key) ?>" name="<?= htmlspecialchars($key) ?>" value="<?= htmlspecialchars($row[$key]) ?>">
    </div>
  <?php endforeach; ?>
  <button type="submit" class="btn btn-primary" id="submit-btn">Submit</button>
  <button type="button" class="btn btn-secondary" onclick="window.location.href='wew.php';">Back</button>
</form>
                </div>
              </div>
            </div>
